...
本文唯一拥有 `dsh-guardrails` 对 pwsh 命令文本的词法、动词分类、列举模式判定、破坏性分析与固有限制。规则模型与名单归 `规则模型.md`，需求归 `../需求.md`。
...
- `CRED_TEXT_REFERENCE`：凭据文件名/目录段/组合的文本出现（`.pub` 排除）；`.dsh`（含会话历史）不在任何引用规则中（见 DSR-003 重访注记，规则模型.md）。
